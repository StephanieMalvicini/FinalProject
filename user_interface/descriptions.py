...
class DescriptionsContainer:
    ...
    def __init__(self, main_frame, width):
        self.frame = ttk.LabelFrame(main_frame, text="  Atributos descripciones  ", height=150, width=width)
...
class Descriptions:
    ...
    def create_check_buttons(self):
        ...
        for i, name in enumerate(attributes_names):
            value = tk.BooleanVar()
            value.set(False)
            check_button = ttk.Checkbutton(self.frame, text=name, var=value, width=28)
            if i % AMOUNT_PER_ROW == 0:
                column = 0
                self.row += 1
            else:
                column += 1
            check_button.grid(column=column, row=self.row)
